...
def inject_noise(input_embeds, vit_embeds, selected, B, N, C):
    input_embeds_cd = input_embeds.clone()
    vit_embeds_cd = add_diffusion_noise(vit_embeds, 500)

    try:
        input_embeds_cd[selected] = input_embeds_cd[
            selected
        ] * 0.0 + vit_embeds_cd.reshape(-1, C)
    except Exception as e:
        vit_embeds_cd = vit_embeds_cd.reshape(-1, C)
        print(
            f"warning: {e}, input_embeds[selected].shape={input_embeds_cd[selected].shape}, "
            f"vit_embeds_cd.shape={vit_embeds_cd.shape}"
        )
        n_token = selected.sum()
        input_embeds_cd[selected] = (
            input_embeds_cd[selected] * 0.0 + vit_embeds_cd[:n_token]
        )

    input_embeds_cd = input_embeds_cd.reshape(B, N, C)
    return input_embeds_cd
...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # with torch.no_grad():
            #     cd_outputs = self.language_model(
            #         inputs_embeds=input_embeds_cd,
            #         attention_mask=attention_mask,
            #         position_ids=position_ids,
            #         past_key_values=past_key_values,
            #         use_cache=use_cache,
            ...
            # loss = loss.sum() / (
            #     sub[shift_labels != self.img_context_token_id].sum() + 1e-6
            # )
